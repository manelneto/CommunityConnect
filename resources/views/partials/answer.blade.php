<form class="answer" method="post" enctype="multipart/form-data">
    ...
    <article class="content-right">
        ...
        @if (Auth::user()?->id === $answer->id_user || Auth::user()?->administrator)
            @if ($answer->correct)
                <button data-id="{{ $answer->id }}" class="mark-incorrect" formaction="../../answers/{{ $answer->id }}/incorrect">Remove correct mark</button>
            @else
                <button data-id="{{ $answer->id }}" class="mark-correct" formaction="../../answers/{{ $answer->id }}/correct">Mark as correct</button>
            @endif
